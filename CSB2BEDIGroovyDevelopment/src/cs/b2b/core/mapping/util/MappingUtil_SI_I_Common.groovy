...
class MappingUtil_SI_I_Common {
	...
	public List<Object> buildBizKey(MarkupBuilder bizKeyXml, def current_Body,int current_BodyIndex, def errorKeyList, String TP_ID, Connection conn) {
		...
		//L3STPID
		bizKey.add(["L3STPID" : TP_ID])

		// RTPID & L3RTPID
		if (util.notEmpty(current_Body.GeneralInformation.SCAC)){
			bizKey.add(["RTPID" : util.getCarrierTpId(TP_ID, 'SI', current_Body.GeneralInformation.SCAC.text(), conn)])
			bizKey.add(["L3RTPID" : util.getCarrierTpId(TP_ID, 'SI', current_Body.GeneralInformation.SCAC.text(), conn)])
		}
