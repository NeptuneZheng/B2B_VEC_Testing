...
import cs.b2b.core.mapping.bean.bl.DepartureDT;
...
class MappingUtil_BL_O_Common {
	 ...
	 * @RuleName FilterByChargeTypeAccept
	 */
	void filterByChargeTypeAccept(List<FreightCharge> freightChargeList, List<FreightChargeCNTR> freightChargeCNTRList, List<String> acceptChargeTypeList){
		if(freightChargeList){
			Iterator<FreightCharge> iterator = freightChargeList.iterator();
			while(iterator.hasNext()){
				FreightCharge freightCharge = iterator.next();
	 ...
	 * @author XIEMI
	 */
	void collectCurrencyNotUSDCAD(cs.b2b.core.mapping.bean.bl.Body current_Body, boolean isError, String errorMsg, List<Map<String,String>> errorKeyList){
		Map<String, String> errorKey = null;
		
		def fcList = current_Body?.FreightCharge?.findAll { current_fc ->
			current_fc?.ChargeType == '0'
		}
		
		def cnt = fcList.findAll{ current_fc ->
			current_fc?.ChargeAmount?.attr_Currency == "USD" || current_fc?.ChargeAmount?.attr_Currency == "CAD"
		}.size();
